...
- Property mapping names must be unique in Authentik. Inline mappings default their name to `scope_name`, or to a sanitized version of `saml_name` (non-`[-a-zA-Z0-9_]` characters replaced with `_`) when no explicit `name` is set; set an explicit `name` if you need to disambiguate.
