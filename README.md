...
> **Enterprise only:** the Microsoft Entra provider synchronizes users and groups from Authentik to Microsoft Entra ID and requires an [Authentik Enterprise license](https://goauthentik.io/pricing/). All other providers are available in the open-source edition. See the [Enterprise features](https://docs.goauthentik.io/enterprise/enterprise-features/) documentation for details.
...
| [`examples/complete`](examples/complete) | One application per supported protocol: OAuth2, SAML + SCIM backchannel, proxy, LDAP, RADIUS, WS-Federation, and Microsoft Entra |
| [`examples/advanced`](examples/advanced) | Kitchen-sink: self-generated signing/encryption key pairs (`tls` provider), inline property mappings, provider tuning options, and a SCIM backchannel |
